...
const shiftString = (str, shift) => {
    ...
    const shiftedCharList = charList.map((c) => shiftChar(c, shift));
    return shiftedCharList.join('');
};
...
const decipher = () => {
  ...
  const result = caesarDecipher(cipherText, shift);
  document.getElementById('result').textContent = 'Deciphered Text: ' + result;
};

const button = document.getElementById('decipher-button');
button.addEventListener('click', decipher);
