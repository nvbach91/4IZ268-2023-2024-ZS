...
const shiftChar = (c, shift) => {
    if (alphabet.includes(c)) {
        const index = (alphabet.indexOf(c) - shift + alphabet.length) % alphabet.length;
...
const decrypt = () => {
    ...
    const decryptedText = shiftString(encryptedText, shiftValue);
    document.getElementById('decryptedText').textContent = decryptedText;
};

function setupDecryptionButton() {
    const button = document.getElementById('decryptButton');
    button.addEventListener('click', decrypt);
}
document.addEventListener('DOMContentLoaded', setupDecryptionButton);