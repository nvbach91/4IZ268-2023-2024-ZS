...
export const ContactForm: FunctionComponent<ContactFormProps> = ({ formFields, availibilityDates }) => {
	...
	const mutation = api.contact.send.useMutation({
		onSuccess: () => {
			...
			setFormData({ name: '', email: '', tel: '', date: availibilityDates[0].id, numberOfPeople: '1' })
	...
	const dateOptions = useMemo(() => {
		return availibilityDates
			.map((availibilityDate) => {
				return availibilityDate.numberOfPeople > availibilityDate.numberOfOccupiedPlaces
					? ({
							value: availibilityDate.id,
							label: `${new Date(availibilityDate.dateTime).toLocaleDateString('cs-CZ', {
								day: 'numeric',
								month: 'numeric',
								year: 'numeric',
								hour: 'numeric',
								...
								availibilityDate.numberOfPeople - availibilityDate.numberOfOccupiedPlaces
							} `,
				...
				>
					...
					<div className={styles.submitButtonWrapper}>
						<div className={styles.submitButton}>
							<Button type="submit" size="medium" variant="primary" disabled={isPending} isFullWidth>
