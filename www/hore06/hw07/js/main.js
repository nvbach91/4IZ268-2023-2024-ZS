...
const shiftChar = (c, shift) => {
    ...
    const start = alphabet.indexOf(c);
    if (start === -1){
        return c;
    }
    const maybyShift = start-shift;
    if (maybyShift < 1){
        const newShift = 26+maybyShift;
        return alphabet.charAt(newShift);
    } 
    else{
        return alphabet.charAt(maybyShift);
    }

};
//console.log(shiftChar('Z',4));
...
const decipher = () =>{
    ...
    document.getElementById('resultSpace').textContent= caesarDecipher(text,key);
